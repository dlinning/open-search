...
export interface ISearchProvider<TRawReq = unknown, TRawRes = unknown> {
	...
	search(request: SearchRequestDto, searchId: string, boostParams?: QueryBoostParameters): Promise<SearchResponseDto>;
...
export abstract class BaseSearchProvider<TRawReq, TRawRes> implements ISearchProvider<TRawReq, TRawRes> {
	...
	public abstract mapToVendorRequest(request: SearchRequestDto, boostParams?: QueryBoostParameters): TRawReq;
	...
	public async search(
		...
		try {
			...
			const executionTimeMs = Math.round(performance.now() - startTime);
			throw new AppError(`Provider ${this.providerId} execution failed: ${err.message}`, 502, "PROVIDER_ERROR", {
...
export class PersonalizationService {
	...
	public async getSessionBoosts(userContext: UserContext): Promise<QueryBoostParameters | undefined> {
...
const envSchema = z.object({
	...
	DEFAULT_PROVIDER: z.enum(["algolia", "commercetools", "gcp_vertex_search", "opensearch", "mock"]).default("mock"),
